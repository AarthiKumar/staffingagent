...
Both pathways are feasible in this codebase:

- **Pathway A (strict identity)** is immediately feasible with current schema (`email`, `phone`) and existing duplicate checks.
- **Pathway B (probabilistic)** is feasible as an incremental enhancement by extending merge proposal scoring and review workflows, without changing the core candidate/document model.

Recommended rollout:
1. Keep strict identity as the hard auto-link path.
2. Add probabilistic scoring as a review-only path.
3. Promote to optional auto-merge only after acceptance-rate telemetry is stable.
